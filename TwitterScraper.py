...
def assignBrandTag(search_keyword):
    ...
    siggis = ['siggi skyr', 'siggi filmjolk', 'siggi tubes', 'siggi dairy', 'siggi yogurt', 'siggisdairy', '#siggisdairy', '@siggisdairy', "siggi's skyr", "siggi's filmjolk", "siggi's tubes", "siggi's dairy", "siggi's yogurt"]
...
for searchQuery in search_query_list:
    try:
        if searchQuery not in flag_list:
            ...
            for tweet in query_tweets(searchQuery, limit=30000):
                tweet_id = tweet.id.encode('utf-8')
                user = nonasciireplacer(unidecode(tweet.user).encode('utf-8'))
                fullname = nonasciireplacer(unidecode(tweet.fullname).encode('utf-8'))
                timestamp = tweet.timestamp
                text = nonasciireplacer(unidecode(tweet.text).encode('utf-8'))
